...
export function TopDishes({ dishes }: { dishes: TopDish[] }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-5">
      <h2 className="mb-5 font-serif text-lg text-foreground">{t("dashboard.topDishes")}</h2>
      {dishes.length === 0 ? (
          ...
          {dishes.map((dish, i) => (
            <li key={dish.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm text-foreground">{dishName(dish, locale)}</p>
                <p className="text-xs text-muted-foreground">{t("dashboard.portionsSold", { count: dish.quantitySold })}</p>
              </div>
